...
export function SkillRoadmapModal({ skill, isOpen, isMastered, onClose, onMarkMastered }: { skill: SkillItem | null; isOpen: boolean; isMastered: boolean; onClose: () => void; onMarkMastered?: () => void }) {
  ...
  return <div className="fixed inset-0 z-[400] flex items-center justify-center p-4">
    ...
    <div className="relative card w-full max-w-2xl max-h-[88vh] overflow-y-auto p-6 z-10">
      ...
      <div className="space-y-3">{steps.map((step, index) => <div key={step.title} className="rounded-lg border border-line bg-ink-2 p-4"><div className="flex gap-3"><div className="w-8 h-8 rounded-full bg-teal/15 text-teal flex items-center justify-center font-mono text-sm font-bold flex-none">{index + 1}</div><div className="min-w-0"><h3 className="font-display text-lg">{step.title}</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2"><div><div className="font-mono text-[10px] text-amber uppercase">Goal</div><p className="text-xs text-bone-dim mt-1 leading-relaxed">{step.goal}</p></div><div><div className="font-mono text-[10px] text-teal uppercase">Practice</div><p className="text-xs text-bone-dim mt-1 leading-relaxed">{step.practice}</p></div></div><div className="flex items-start gap-2 mt-3 text-xs text-bone"><Check size={14} className="text-teal flex-none mt-0.5" /><span><span className="font-semibold">Checkpoint:</span> {step.checkpoint}</span></div></div></div></div>)}</div>
