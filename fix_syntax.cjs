...
for (const ex of toAdd) {
    const rx = new RegExp(\`name:\\s*'\${ex.name}'[\\s\\S]*?secondaryMuscles:\\s*,[\\s\\S]*?tags:\\s*\\[\\]\`);
    content = content.replace(rx, \`name: '\${ex.name}',\n    muscleGroup: '\${ex.muscleGroup}',\n    secondaryMuscles: [\${ex.secondaryMuscles.map(m=>\`'\${m}'\`).join(', ')}],\n    equipment: 'Bodyweight',\n    difficulty: 'intermediate',\n    tags: []\`);
}

fs.writeFileSync('src/services/library.ts', content);
console.log('Fixed syntax in library.ts');
